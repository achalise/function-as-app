...
fun checkEligibilityFn(repository: ApplicationRepository): (application: String)-> String {
    return {
        val event = objectMapper.readValue(it, Claim::class.java)
        val eventOut = event.copy(status = ApplicationState.APPLICATION_STATE_ELIGIBILITY_CHECK_SUCCESS.name)
