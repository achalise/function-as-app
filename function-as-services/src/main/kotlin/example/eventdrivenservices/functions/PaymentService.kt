...
fun processPaymentFn(repository: ApplicationRepository): (application: String)-> String {
    return {
        val event = objectMapper.readValue(it, Claim::class.java)
        val eventOut = event.copy(status = ApplicationState.APPLICATION_STATE_PAYMENT_SUCCESSFUL.name)
